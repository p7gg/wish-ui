 ...
 * MIT Licensed, Copyright(c) 2021 SEEK, see LICENSE.seek.md for details
 *
...
import { vanillaCssTsFilesLoader } from './vanillaCssTsFilesLoader'
...
export function vanillaExtractPlugin({ identifiers, esbuildOptions }: Options = {}): Plugin {
  ...
  esbuildOptions.plugins = [...(esbuildOptions?.plugins ?? []), ...coreEsbuildPlugins]

  // eslint-disable-next-line @typescript-eslint/no-inferrable-types, turbo/no-undeclared-env-vars
  ...
  return {
    ...
    async configResolved(resolvedConfig) {
      ...
      if (
        config.plugins.some((plugin) => ['astro:build', 'solid-start-server'].includes(plugin.name))
      ) {
